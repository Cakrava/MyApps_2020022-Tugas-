...
export default function ModalDataMatakuliah({
  ...
  const fetchDataMatakuliah = async (pageNumber = 1, searchQuery = search) => {
    ...
    try {
      const response = await fetch(
        `${apiMahasiswa}matakuliah?page=${pageNumber}&search=${searchQuery}`,
        {
          headers: {
  ...
  const pilihDataMatakuliah = item => {
    onMatakuliahSelected(item.mtkkode, item.mtknama);
  };
  ...
  const renderItemMatakuliah = ({item}) => (
    <TouchableOpacity
      ...
      <Text>{item.mtknama}</Text>
      <Text>{item.mtkkode}</Text>
    </TouchableOpacity>
  );
  return (
    <Overlay
      ...
      {initialLoading ? (
        ...
        <FlatList
          ...
          keyExtractor={item => item.mtkkode}
          onEndReached={() => {
